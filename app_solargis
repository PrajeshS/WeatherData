...
st.title("☀️ SolarGIS Weather Forecast Data Analysis")
...
DATA_DIR = '.'
FORECAST_FILE = "solargis_forecast.csv"

FORECAST_MAPPING = {
    "Albedo Up Irradiance (W/m²)": "GHI",
    "ATRHP Temperature (°C)": "TDRY",
    "ATRHP Humidity (%)": "RH"
}
...
avg_sensor = avg_sensor.reset_index()
...
DISPLAY_OPTIONS = {
    ...
    "ATRHP Temperature vs TDRY (°C)": "ATRHP Temperature (°C)",
    "ATRHP Humidity vs RH (%)": "ATRHP Humidity (%)"
}
...
comparison = comparison.drop_duplicates(subset=["Time"])
...
if param == "Albedo Up Irradiance (W/m²)":
