    ...
    for folder in sensor_folders:
        ...
        try:
            ...
            for display_name, possible_names in target_params.items():
                # Look for a column that exactly matches or contains one of our keys
...
if not common_dates:
    ...
    if not data.empty:
        with st.expander("Debug: Column Mapping Check"):
            st.write("Available Parameters:", [c for c in data.columns if c not in ['Time', 'Sensor']])
