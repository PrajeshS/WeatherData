...
def load_and_preprocess_data(base_dir, sensor_folders, selected_date, required_columns):
    ...
    for folder in sensor_folders:
        ...
        try:
            ...
            # Clean headers: remove quotes, strip whitespace
...
if not common_dates:
    st.warning('No common dates found. Please check folder names.')
else:
    ...
    if not data.empty:
        with st.expander("Data Preview & Column Names"):
            st.write("Available columns from CSV headers:", data.columns.tolist())
            st.dataframe(data.head())
            
        param = st.selectbox('Choose Parameter', REQUIRED_COLUMNS)
        plot_data = data.dropna(subset=[param])

        if not plot_data.empty:
            st.subheader(f'Overlap: {param}')
            st.plotly_chart(px.line(plot_data, x='Time', y=param, color='Sensor'), use_container_width=True)

            st.subheader(f'Average: {param}')
            ...
            if s1 != s2:
                ...
                if not comp.empty:
                    ...
                    st.info('No overlapping timestamps between these sensors.')
        else:
            st.error(f'No numeric data found for {param}. Check the Data Preview above.')
    else:
